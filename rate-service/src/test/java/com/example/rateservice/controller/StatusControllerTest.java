...
class StatusControllerTest {
    ...
    void shouldAlwaysReturnConsistentStatus() {
        ...
        assertThat(status1.get("status")).isEqualTo("UP");
    }
    ...
    @Test
    @DisplayName("Should return exactly one entry")
    void shouldReturnExactlyOneEntry() {
        // When
        Map<String, String> status = statusController.getStatus();

        // Then
        assertThat(status).hasSize(1);
        assertThat(status).containsOnlyKeys("status");
    }
}
